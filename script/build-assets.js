...
records.forEach((item, index) => {
  ...
  const boothObj = {
    ...
    tags: item.tags.replace("展示", "exhibitioin").replace("パフォーマンス", "performance").replace("ゲーム・体験", "game/experience").replace("食品", "food").replace("部誌販売(配布)", "book").replace("販売", "store").replace("アトラクション", "attraction").replace("映像", "movie").split(", "),
    color: item.color,
    has_cm: item.cm === "true" ? true : false,
    has_website: item.has_website === "true" ? true : false,
    group_name: (() => {
      return index_records.find(ite => ite.booth_id === item.booth_id)?.group_name
    })(),
    // place_id: 
    place: (() => {
    ...
    overview: (() => {
      return index_records.find(ite => ite.booth_id === item.booth_id)?.overview
    })(),
  }
  const contentJson = JSON.stringify(boothObj);
  writeFileSync(process.cwd() + OUTPUT_DIR_PATH + "/" + item.booth_id + ".json", contentJson, {
    encoding: "utf8",
  });
})

// allIds.txt
const allIds = records.map(item =>
  item.booth_id
).reduce((acc, curr) => {
  return acc + "," + curr
});
